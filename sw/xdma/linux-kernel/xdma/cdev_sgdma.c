		...
		size_t count, loff_t *pos, bool write)
{
	int rv;
	ssize_t res = 0;
	struct xdma_cdev *xcdev = (struct xdma_cdev *)file->private_data;  // custom device struct
	struct xdma_dev *xdev;
	struct xdma_engine *engine;
	struct xdma_io_cb cb;

	rv = xcdev_check(__func__, xcdev, 1);  // check magic
	if (rv < 0)
		return rv;
	xdev = xcdev->xdev;
	engine = xcdev->engine;
	...
	rv = check_transfer_align(engine, buf, count, *pos, 1);  // check AXI Addr align (MM INCR)
	if (rv) {
